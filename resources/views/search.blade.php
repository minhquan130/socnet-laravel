...
<div class="wrapper-search">
    <div class="filter-search">
        <ul class="options-filter">
            <h4>Bộ lọc</h4>
            <li class="item-option-filter {{ url()->current() == asset('').'search' ? 'active-filter' : '' }}">
                <a href="{{ route('search', ['keyword' => request('keyword')]) }}">Tất cả</a>
            </li>
            <li class="item-option-filter {{ str_contains(url()->current(), asset('').'search/all-users') ? 'active-filter' : '' }}">
                <a href="{{ route('search.allUsers', ['keyword' => request('keyword')]) }}">Mọi người</a>
            </li>
            <li class="item-option-filter {{ str_contains(url()->current(), asset('').'search/all-posts') ? 'active-filter' : '' }}">
                <a href="{{ route('search.allPosts', ['keyword' => request('keyword')]) }}">Bài viết</a>
            </li>
        </ul>
    </div>
    <div class="result-search">
        @if (!empty($resultContentPost) && !empty($resultUsername))
            @if ($resultUsername->count() === 0 && $resultContentPost->count() === 0)
        ...
        @if (!empty($resultUsername))
            @if ($resultUsername->count() !== 0)
                <div class="friend-search">
                    <h2>Mọi người</h2>
                    @php
                        $countUser = $resultUsername->count();
                        $resultUsername->pop();
                    @endphp
                    ...
                    @endforeach
                    @if ($countUser === 4)
                        <a href="{{ route('search.allUsers', ['keyword' => request('keyword')]) }}">
                            <button class="btn-show-all">Xem thêm tất cả</button>
                        </a>
                    @endif
                </div>
            @endif
        @endif
        @if (!empty($resultContentPost))
            @if ($resultContentPost->count() !== 0)
                                                        ...
                                                        @if ($post->sharedPost)
                                                            <div
                                                                ...
                                                                <img src="{{ $post->sharedPost->profile_pic_url == null ? asset('images/none-avatar.jpg') : $post->sharedPost->profile_pic_url }}"
                                                                    ...
                                                                    style="width: 50px; height: 50px; border-radius: 50%">
                                                                <span
                                                                    class="name">{{ $post->sharedPost->username }}</span>
                                                            </div>
                                ...
                                <div class="metrics-post">
                                    ...
                                    <div class="count-comments">
                                        ...
                                        <a class="option option-comment" data-bs-toggle="modal"
                                            ...
                                            <span id="qty-comments-post-{{ $post->post_id }}">{{ $countComments }}
                                                bình
                                                luận</span>
                                        </a>
                                    </div>
                                ...
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        ...
                                        <form action="{{ route('posts.share', $post->post_id) }}" method="POST"
                                            ...
                                            <input type="hidden" name="shared_post_id"
                                                value="{{ $post->post_id }}">
                                            <div class="modal-footer">
                                                <button type="button" class="btn btn-secondary"
                                                    data-bs-dismiss="modal">Hủy</button>
